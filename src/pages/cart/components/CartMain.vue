...
const isSelected = computed(() => {
  return !cartList.value.length ? false : cartList.value.every((v) => v.selected)
})
// 计算商品全选 状态.value
const onChangeSelectedAll = async (isAll: boolean) => {
  cartList.value.length && cartList.value.filter((v) => (v.selected = !isAll))
  await putMemberCartSelectedAPI({ selected: !isAll })
}
// 计算选 中单品列表
...
const selectedCartListCount = computed(() => {
  return selectedCartList.value.reduce((pre, cur) => pre + +cur.count, 0)
})
// 根据选中商品列表计算 总价格
  ...
  <scroll-view @scrolltolower="onScrolltolower" scroll-y class="scroll-view">
    ...
    <template v-if="memberStore.profile">
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        <text
          @tap="($event) => onChangeSelectedAll(isSelected)"
          class="all"
